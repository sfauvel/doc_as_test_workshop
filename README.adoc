...
====
We start, by some basics about `Doc as code`: write a doc with a markup language, store it with source control, add tools to visualize it. 
====
...
Install an `asciidoc` viewer to view the rendering of an asciidoc file.
...
Write a asciidoc file describing the current workshop.
Try to format it to be pleasant to read (title, bold, link, table, ...). 

You can found some basic syntaxes examples here: link:pages/asciidoc.{page_extension}[]
...
You also can look at the page link:work/asciidoc_to_html.{page_extension}[Asciidoc to HTML] which describes alternative ways of doing things.

[%collapsible]
.Publish site with Github action
======
include::pages/asciidoc_to_html.adoc[leveloffset=+1,tags=github-configure]
include::pages/asciidoc_to_html.adoc[leveloffset=+1,tags=github-create-action]
include::pages/asciidoc_to_html.adoc[leveloffset=+1,tags=github-publish]
======
...
* Use a test from which you will create a file containing some information.

* Write in a file the list of Parrot available. 

* Add the speed of each Parrot build with default values.

* Improve the text to produce a document that allows the behavior described to be understood.
...
[source, python]
----
remove received_file if exists

if approved_file not exists or approved_content != received_content:
...
We want to create a `road` along which the `parrot` can travel. 
This`road` will contain the `parrot` position and `power source` in some places.
A `power source` has a float value between 0 to 2.

Start by create a formatter that display the state of the system:

- The road.
- Position of the `parrot` and its speed.
- Items along the `road`.

We can call a method to move forward the `parrot`.
After 5 steps, its voltage decrease by 0.1.
When a `parrot` passes over a `power source`, it takes all its energy. 

Don't try to make something to sophisticated. 
We want to work on the representation more than the code design.
It doesn't matter if the code is far from good practice, that's the subject to experiment with.
