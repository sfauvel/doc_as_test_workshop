...
In short, each test method generates a paragraph describings a behaviour. 
...
You can found some basic syntaxe here: link:asciidoc.{page_extension}[]
...
You also can look at the page link:asciidoc_to_html.adoc[Asciidoc to HTML] which describes alternative ways of doing things.
...
For that, we need a program that that executes the code to retrieve behaviour.
